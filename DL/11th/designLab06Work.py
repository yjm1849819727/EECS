...
def bestk2(k1, k2Min, k2Max, numSteps):
    T = 0.1
    V = 0.1
    return optimize.optOverLine(lambda k2 : max(poly.Polynomial([1,-2,V*T**2*k1+1,V*T**2*k2]).roots(),key=abs),k2Min,k2Max,numSteps)
...
def anglePlusPropModel(k3, k4):
    ...
    plant2 = sf.Cascade(sf.Cascade(sf.R(), sf.Gain(V*T)), sf.FeedbackAdd(sf.Gain(1), sf.R()))
    # The complete system
    sys = sf.FeedbackSubstract(sf.Cascade(sf.Cascade(sf.Gain(k3),sf.FeedbackSubstract(plant1,sf.Gain(k4))),plant2),sf.Gain(1))
